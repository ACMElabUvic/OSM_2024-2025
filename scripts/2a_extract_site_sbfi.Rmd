...
Now we can load in and clip the geodatabase containing the 2020 SBFI. This takes quite a while depending on file size. 

```{r}
# load landcover layer (will be slow)
landcover_unclipped <- st_read("C:/databases/SBFI2020_Boreal_Plains.gdb", layer = "SBFI_2020_polygons")
...
save(sites_landcover_metrics, file = "./data/input/OSM_SBFI2020_site_covariates.RData")
```
...
sites_landcover_metrics_df <- bind_rows(sites_landcover_metrics) %>%
  ...
  relocate(array, site, array_visit, buffer_dist) %>%
  
  select(-lat, -long)

write_csv(sites_landcover_metrics_df, "./data/input/OSM_SBFI2020_site_covariates.csv")
```
