...
Let's read in the data from script 3. For now, we'll focus this entire script on just the 2024-2025 data since that's what we want for the Fiscal report. Importing the *entire* dataset means we can reuse this code for the data paper, or project-wide summaries as needed without major revision. 
...
What sites do we want to visualize? If we constrain this to sites contained within "independent detections" file we'll be able to filter everything to only the sites for which we actually have data. 
...
## 2.1. Indpendent detections for mammals
...
## 2.2. Independent detections per LU
...
## 2.3. Naive occupancy by species/array
...
fig_naiveocc <- totaldet %>%
  ...
  # Only the 2024-2025 sites identified early in the process
  filter(site %in% sites) %>%
