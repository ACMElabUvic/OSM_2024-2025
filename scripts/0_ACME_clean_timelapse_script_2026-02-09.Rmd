...
Also make sure you open RStudio through the R project (OSM_2024-2025.Rproj) this will automatically set your working directory to the correct place (wherever you saved the repository) and ensure you don't have to change the file paths for some of the data. In the *Global Options* of your RStudio, ensure chunks are set to run in the *project directory*, not the *default.* 
...
```{r script_parameters}
my_netdrive <- 'Z:/3. ACME Projects/1. Alberta/OSM/2024-2025' # the path to the netdrive on your device
...
## 2.1. Read in and reformat the deployment data. **The initial step of reading/copying over from the Netdrive into the data/raw folder was performed manually in 2024. The correctly formatted sheets are in the Github repo already!!** 

```{r, eval = TRUE}
OSM_2024_deploy_raw <- read_csv("./data/raw/OSM_deployment_2024.csv", 
                                col_types = cols(
...
# check data
head(OSM_2024_deploy_raw)

# check levels of site name
levels(OSM_2024_deploy_raw$site)

# create graph of camera operability
ggplot(OSM_2024_deploy_raw, aes(color = array))+
...
```{r fix, eval = FALSE}
OSM_2024_deploy <- OSM_2024_deploy_raw %>% 
...
# check data
summary(OSM_2024_deploy)

# check levels of site name
levels(OSM_2024_deploy$site)

# create graph of camera operability
ggplot(OSM_2024_deploy, aes(color = array)) +
...
```{r export, eval = FALSE}
write_csv(OSM_2024_deploy, "./data/processed/OSM_deployment_2024.csv")

rm(OSM_2024_deploy_raw)
```
...
OSM_2024_deploy <- read_csv("./data/processed/OSM_deployment_2024.csv") %>% 
  # make sure site re-reads in as a factor to compare with other data sets
  mutate(site = as.factor(site))
```
...
save(OSM_2024_timelapse_ddb_files, file="./data/raw/OSM_2024_timelapse_ddb_files.Rdata")
```
...
load("./data/raw/OSM_2024_timelapse_ddb_files.Rdata")
```
...
# Then replace NAs with 0s in the species count columns for easier error checking
OSM_2024_data <- OSM_2024_data %>%
  mutate(
...
if(nrow(OSM_2024_error_duplicate) == 0){
  ...
  writexl::write_xlsx(OSM_2024_error_duplicate, "./data/processed/OSM_2024_error_duplicate.xlsx", format_headers = TRUE)
...
Identify sites with a seasonally improbable value for snow cover:

First, winter
```{r}
error_snow_winter <- OSM_2024_data %>%
  
  mutate(month=month(datetime)) %>% # extract month as a helper column
  
  filter(month %in% c(11, 12, 1, 2, 3, 4)) %>%
  
  group_by(site, datasource) %>%
  
  summarize(all_snow_is_zero = all(snow == "0")) %>%
  
  filter(all_snow_is_zero) %>%
  
  select(site, datasource) %>%
  
  mutate(error = "All winter snow values are zero")
```

Then, summer
```{r}
error_snow_summer <- OSM_2024_data %>%
  ...
  filter(all_snow_is_zero == FALSE) %>%
  
  select(site, datasource) %>%
  
  mutate(error = "Snow is tagged as present in the summer")
```
...
```{r}
error_datamissing <- OSM_2024_deploy %>%
  filter(site %in% setdiff(levels(OSM_2024_deploy$site), # check if there are extras in deployment data compared to timelapse
                           levels(OSM_2024_data$site))
        ) %>%
...
```{r}
error_deploymissing <- OSM_2024_data %>%
  filter(site %in% setdiff(levels(OSM_2024_data$site), # check if there are extras in Timelapse data compared to deployment
                           levels(OSM_2024_deploy$site))
        ) %>%
...
Combine all the errors into a single spreadsheet and export to the top level of the repo. This is easy to find, and we'll remember to delete them in later commits. 
```{r}
...
rm(error_classifier, error_snow, error_site, error_snow_winter, error_snow_summer, error_datamissing, error_deploymissing) # clean up
```
...
error_framecount_missing <- OSM_2024_data %>%
  ...
  # All frame counts are NA or 0
  filter(if_all(c(total, male, female, unknownsex, adult, yly, yoy, unknownage), 
                ~is.na(.) | . == 0) ) %>% # are all of them NA?
...
Any frame totals for a given image exceed group totals:
```{r}
error_framecount_low <- OSM_2024_data %>%
  ...
  filter(
    (max(group_count) > sum(total)) |
    (max(g_male) > sum(male)) |
    (max(g_female) > sum(female)) |
    (max(g_unknownsex) > sum(unknownsex)) |
    (max(g_adult) > sum(adult)) |
    (max(g_yly) > sum(yly)) |
    (max(g_yoy) > sum(yoy)) |
    (max(gunknownage) > sum(unknownage))
  ) %>%
  # If the event total indicates a species is present, but the animal is never actually present in a 
  
  mutate(
    error = "Group count exceeds summed frame counts."
  )
...
```{r}
# Missing coat colour
error_missingcoatcolor <- OSM_2024_data %>%
...
Coat colour on a species that shouldn't have it:
```{r}
# Missing coat colour
error_extracoatcolor <- OSM_2024_data %>%
  
  filter(
    species %nin% c(NA, 'Black bear', 'Red fox', 
                    'Grey wolf', 'Grizzly bear', 
                    'Unknown canid', 'Unknown bear') # the only species of interest for coat color. 
    ) %>%
  
  filter(
    !is.na(coatcolour) # is coat color NA?
    ) %>%
  
  mutate(
    error = "Coat color present on a species that should not have coat color"
    )
```
...
```{r}
error_missingantlercount <- OSM_2024_data %>%
...
Antler count on a species that shouldn't have it:
```{r}
# Missing coat colour
error_extraantlercount <- OSM_2024_data %>%

  
  filter(
    species %nin% c(NA, 'Moose',
                    'Caribou', 'White-tailed deer',
                    'Mule deer', 'Unknown ungulate',
                    'Unknown deer') # the only species of interest for coat color. 
    ) %>%
  ...
  mutate(
    error = "Antler counts present on a species that should not have antler counts"
    )
```
...
OtherSpecify is not filled out when something is other:
```{r}
error_otherspecify <- OSM_2024_data %>%
  
  filter(species %in% c("Other", "Other birds") | 
           cameramalfunction == "OtherSpecify" |
           coatcolour == "OtherSpecify" ) %>%
  
  filter(otherspecify %in% c(" ", "") | 
           is.na(otherspecify)) %>%
  
  mutate(
    error = "OtherSpecify field is not filled out"
  )
```


OtherSpecify is filled out, but the species exists as an entry in the CodeTemplate:

```{r}
error_otherspecifyspeciesexists <- OSM_2024_data %>%
  
  # This is a list of the species in the codetemplate
  filter(otherspecify %in% 
           c("White-tailed deer", "Mule deer", "Black bear", "Grizzly bear", 
             "Snowshoe hare", "Arctic hare", "Red squirrel", "Moose", "Grey wolf", 
             "Coyote", "Red fox", "Arctic fox", "Elk", "Wild horse", "Domestic dog", 
             "Cougar", "Lynx", "Wolverine", "Marten", "Fisher", "Mink", 
             "Long-tailed weasel", "Short-tailed weasel", "Striped skunk", "Porcupine", 
             "Hoary marmot", "Groundhog", "Chipmunk", "Beaver", "Otter", "Cow", 
             "Bighorn sheep", "Mountain goat", "Dall sheep", "Muskox", "Caribou", "Human",
             "Staff", "ATVer", "Hiker", "Biker", "Snowmobiler", "Hunter", "Equestrian", 
             "Spruce grouse", "Ruffed grouse", "Sharp-tailed grouse", "Willow ptarmigan", 
             "Raven", "Grey jay", "Canada goose", "Owl", "Eagle", "Sandhill crane", 
             "Other birds", "Unknown", "Unknown mustelid", "Unknown canid", 
             "Unknown felid", "Unknown deer", "Unknown ungulate", "Unknown bear", 
             "Unknown gamebird")) %>%
  
  mutate(
    error = "OtherSpecify field is filled out, but the species exists and should be entered instead"
  )
...
OSM_2024_error_image <- bind_rows(error_speciestag, 
                                  ...
                                  error_framecount_low,
                                  error_framecount_missing, 
                                  error_demographic, 
                                  error_missingcoatcolor, 
                                  error_extracoatcolor,
                                  error_missingantlercount,
                                  error_extraantlercount,
                                  error_emptyevent,
                                  error_missingspecies_untagged,
                                  error_otherspecify,
                                  error_otherspecifyspeciesexists) %>%
  ungroup() %>%
...
```{r}
OSM_2024_error_datemismatch <- OSM_2024_data %>%
  ...
  full_join(OSM_2024_deploy, by='site') %>% # join the site data to the deployment data from Section 2. 
...
if(nrow(OSM_2024_error_datemismatch) == 0){
  writeLines("Lucky you! No date mismatch errors in the dataset.")
} else {
  writexl::write_xlsx(OSM_2024_error_datemismatch, "./OSM_2024_error_datemismatch.xlsx", format_headers = TRUE)
...
Also check that the `otherspecify` values make sense. Are there `OtherSpecify` values that should be in `Comments` instead?
```{r}
OSM_2024_data %>%
...
## 4.6. Export "Unknown" images for manual review:

Some of these could be identified by another staff member and actually not be "unknown". We can export them for a quick review so they can quickly be corrected, or left as unknown as deemed necessary. 

```{r}
unknown_images_to_verify <- OSM_2024_data %>%
  ...
  mutate(
    newdir = paste0("./Unknown species to verify/"), # the location for the image to be exported
...
Then let's export these to the working directory of your choice for review. This could be the Github repo (do not push them), the Netdrive, or your C: drive! This step can be overwritten as many times as needed. 

```{r}
my_errorchecking_wd <- "C:/Users/sdaman/OneDrive - University of Victoria/Documents/GitHub/OSM_2024-2025"
# my_errorchecking_wd <- getwd()
# my_errorchecking_wd <- my_netdrive

with_dir(new = my_errorchecking_wd,

  {purrr::walk(unique(unknown_images_to_verify$newdir), ~dir_create(.x)) # walk suppresses messages for background processes like writing files

  purrr::walk2( # walk suppresses messages for background processes like writing files
    paste0(my_netdrive, "/", unknown_images_to_verify$fullpath), # original path
    paste0(unknown_images_to_verify$newpath), # location of the new folder
...
## 4.7. Export fully-obscured and partially-obscured photos for manual review:

Some of these may be incorrectly tagged due to now-resolved ambiguities in the protocol. We can export them for a quick review so they can quickly be corrected, or left as-is.

```{r}
obscured_images_to_verify <- OSM_2024_data %>%
  
  filter(cameramalfunction %in% c("Fully obscured", "Partially obscured")) %>%

  select(file, datetime, site, cameramalfunction, fullpath, classifier) %>%
  
  mutate(
    newdir = paste0("./Cameramalfunction to verify/", cameramalfunction), # the location for the image to be exported
    newfile = paste0(cameramalfunction, '_', site, '_', # the new, generic file name
                     format(datetime, "%Y%m%d_%H%M%S"),
                     '.JPG'),
    newpath = paste0(newdir, "/", newfile) # the full path to the new file
    )
...
```{r}
my_errorchecking_wd <- "C:/"
# my_errorchecking_wd <- getwd()
# my_errorchecking_wd <- my_netdrive

with_dir(new = my_errorchecking_wd,

  {purrr::walk(unique(obscured_images_to_verify$newdir), ~dir_create(.x)) # walk suppresses messages for background processes like writing files
...
OSM_2024_noon_missing <- OSM_2024_data %>%
...
#         empty = as.logical(TRUE)
  ...
  anti_join(OSM_2024_data, by=c("site", "datetime")) %>% #anti_join(OSM_2024_data, by=c("site", "datetime", "empty")) %>% 
...
Once you've inspected the `OSM_2024_noon_missing` data frame **and confirmed that the photos listed correspond to days where a camera was operating, but no noon photo is present** (as opposed to days cameras were NOT operating), merge them with the OSM data:
```{r, eval=FALSE}
if(autofix_one_image_per_day == TRUE) {     # select/save one image per data and classify species as 'empty' or 'blank' to help quantify survey effort
  ...
  if(tolower(proceed) == 'no'){
    
    
    OSM_2024_data <- OSM_2024_data %>%
        
      bind_rows(., OSM_2024_noon_missing) %>%
      
      arrange(site, datetime)
    
    cat("Noon photos successfully added at the following sites:", paste0(unique(OSM_2024_noon_missing$site)), "\n")

    
  } else {cat("Mission aborted! No noon photos have been added, go back and review your data.\n")}
  }
} else {
  cat("This function is currently disabled.")
}
...
```{r}
OSM_2024_data_clean <- OSM_2024_data %>% 
...
names(OSM_2024_data_clean)
 
# sites (fixed LU 01-71)
levels(OSM_2024_data_clean$site)

print(OSM_2024_data_clean,
      n = 15)
...
write.csv(OSM_2024_data_clean %>% mutate(datetime = paste0(" ", format(datetime, format = "%Y-%m-%d %H:%M:%S"))), 
          file = './data/raw/OSM_timelapse_2024.csv',
          na = "", row.names = FALSE)
...
with_dir(new = my_netdrive, 
  ...
  write.csv(OSM_2024_data_clean %>% mutate(datetime = paste0(" ", format(datetime, format = "%Y-%m-%d %H:%M:%S"))), 
            file = '2. Timelapse Files/OSM_timelapse_2024.csv',
            na = "", row.names = FALSE)
  )
...
with_dir(new = my_netdrive,
  ...
  purrr::walk(paste0('2. Timelapse Files/', unique(OSM_2024_data_clean$array)), 
              ~dir_create(.x))
...
with_dir(new = my_netdrive,
  ...
  purrr::walk(unique(OSM_2024_data_clean$site),
              ~filter(OSM_2024_data_clean, site==.x) %>% 
...
```{r}
noteworthy_photos <- OSM_2024_data_clean %>%
...
```{r}
    OSM_2024_human_to_blur <- OSM_2024_data_clean %>%
...
if(blur_human_use == TRUE){
  ...
  if(tolower(proceed) == 'yes'){
    ...
    with_dir(new = my_netdrive,
             ...
             purrr::walk(OSM_2024_human_to_blur$fullpath,
                        function(fullpath){
