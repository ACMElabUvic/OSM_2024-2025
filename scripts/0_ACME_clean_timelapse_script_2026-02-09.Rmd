...
#my_netdrive <- 'C:/Users/aidanbrushett/Desktop/QAQC done'
...
```{r export, eval = FALSE}
write_csv(OSM_2024_deploy,
          './data/raw/OSM_deployment_2024.csv')
...
OSM_2024_deploy <- read_csv('data/raw/OSM_deployment_2024.csv') %>% 
  # make sure site re-reads in as a factor to compare with other data sets
  mutate(site = as.factor(site))
```
...
error_framecount_low <- OSM_2024_data %>%
  ...
  mutate(
    error = "Group count exceeds summed frame counts, logically impossible."
  )
