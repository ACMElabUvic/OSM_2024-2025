...
osm_deploy_multi_year <- list.files(path="./data/raw/", 
  ...
  mutate(site = as.factor(site),
         array = as.factor(array),
         array_visit = paste0(array, "_", year(start_date))) %>%
...
Good enough. Andrew's data paper code would be very valuable here to get this cleaned up!
...
osm_operating <- osm_coords_deploy %>% 
  ...
  select(array, array_visit, site, date)
```
...
# write_csv(osm_operating, "C:/Users/sdaman/OneDrive - University of Victoria/Documents/GitHub/OSM_2024-2025/data/raw/OSM_operating_days.csv")
```
...
## 5.1. Add the `array_visit` variable to the images file. 


Finally, let's add the `array_visit` variable to this dataset so that we can keep track of which fiscal year deployment took place in. 

```{r}
osm_operating_temporary <- osm_operating %>%
  distinct(array, array_visit, date)

osm_images <- osm_images %>%
  
  mutate(date = date(datetime)) %>%
  
  left_join(osm_operating_temporary,
            by = c("date", "array")) %>%
  
  select(-date) %>%
  
  relocate(array_visit, .after="array")

rm(osm_operating_temporary)
```

Now we can export the imagery:
...
osm_indet <- detections %>% 
  ...
  group_by(array, array_visit, species, site, event_id, year, month) %>%
...
operating_months <- osm_operating %>%
  ...
  group_by(array, array_visit, site, month, year) %>%
...
indet_months <- osm_indet %>%
  ...
  group_by(array, array_visit, site, species, month, year) %>%
...
presence_months <- operating_months %>%
  ...
  left_join(indet_months, by = c("array", "array_visit", "site", "species", "month", "year")) %>%
...
proportional_months <- presence_months %>%
  ...
  group_by(array, array_visit, site, species) %>%
...
operating_months_bears <- operating_months %>%
  ...
  group_by(array, array_visit, site) %>%
...
proportional_months <- proportional_months %>%
  ...
  left_join(operating_months_bears, by = c('array', 'array_visit', 'site')) %>%
...
operating_weeks <- osm_operating %>%
  ...
  group_by(array, array_visit, site, week) %>%
...
indet_weeks <- osm_indet %>%
  ...
  group_by(array, array_visit, site, week, species) %>%
...
presence_weeks <- operating_weeks %>%
  ...
  left_join(indet_weeks, by = c("array", "array_visit", "site", "species", "week")) %>%
...
proportional_weeks <- proportional_weeks %>%
  ...
  left_join(operating_weeks_bears, by = c('array', 'array_visit', 'site')) %>%
...
total_detections <- osm_operating %>%
  ...
  crossing(species = unique(total_indet$species)) %>%
  
  left_join(total_indet, by = c("array", "array_visit", "site", "species")) %>%
